...
if ($current.Count -eq 0) { throw 'Only an active preference can be disabled.' }
...
$disabled = [ordered]@{ preference_id="pref-$PreferenceKey-v$nextVersion"; key=$PreferenceKey; version=$nextVersion; value=$current[0].value; state='disabled'; domain=$current[0].domain; confidence=$current[0].confidence; source='user_disable'; source_candidate_id=$current[0].source_candidate_id; supersedes_preference_id=$current[0].preference_id; observation_count=$current[0].observation_count; user_confirmed=$current[0].user_confirmed; last_used_at=$current[0].last_used_at; updated_at=$now.ToString('o'); expires_at=$current[0].expires_at; disabled_at=$now.ToString('o') }
$newStore = [ordered]@{ schema_version=2; profile='personal'; preferences=@($updated + [PSCustomObject]$disabled); updated_at=$now.ToString('o') }
[IO.File]::WriteAllText($storePath, ($newStore | ConvertTo-Json -Depth 8), [Text.UTF8Encoding]::new($false))
$result = [PSCustomObject]@{ status='preference_disabled'; preference_key=$PreferenceKey; version=$nextVersion; rollback_available=$true; selected_by_runtime=$false; permission_changed=$false; data_scope_changed=$false }
