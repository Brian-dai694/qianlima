...
来自 2026-06-25 的阅读笔记里，Loop Engineering 被拆成三部分：

- `builder`：只负责修复问题
- `checker`：只负责检查，不允许修改
- `loop`：负责编排 builder 和 checker 的循环

核心做法不是靠提示词限制，而是靠工具权限隔离。
...
升级时要带上当前轮次、失败项、已尝试方法、为什么继续循环也不会解决。
...
对千里马来说，Loop Engineering 的重点是把人工看日报、人工判读、人工回传修改，改成“检查 -> 回传 -> 再验证”的系统循环。

## 和成本的关系

Loop Engineering 不是只优化结果，也优化成本。

千里马的目标不是单纯省 token，而是在不降低来源质量、风险检查和任务成功率的前提下降低 token。
...
4. 数据源越来越规范
5. 模板越来越适合大众用户
6. 相同任务下，读取文件更少但关键来源不丢

在千里马里，它也适用于关键词巡检、广告日报、ASIN 诊断和日报生成。
